...
网页版数据更新：重新抓取全部国家与军种的科技树及详情，逐项核对载具 ID、名称、等级、前置关系、各战斗类型的 BR、研发费用与购买货币。飞机默认显示空战 RB，不再被陆战或海战数值覆盖。前置关系不再按列猜测，也不再跳过 Wiki 列出的 Rank I 前置。

金币、联队载具继续排除在普通研发费用合计之外，但 `wiki` 字段完整保留原始费用与货币，这不代表游戏内免费。组合系统的附属载具关联主车。Wiki 没有提供的费用保留为 `null`，网页显示“未提供”，合计提示“未知”，不冒充 0。Wiki 科技树不提供跨等级解锁数量，网页不再把旧数据中的 0 显示成游戏要求，需以游戏内为准。本次更新只针对网页版，不替换已发布的 v1.0.2 安装包。
...
- 网页版沿 Wiki 前置路线计算；跨等级所需解锁数量需在游戏内确认。
