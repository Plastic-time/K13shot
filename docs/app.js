...
async function api(path) {
  ...
  if (treeMatch) {
    ...
    if (!expected) throw new Error(tr("科技树不在核验清单中"));
    const response = await fetch(`${file}?v=${expected.sha256.slice(0, 16)}`, { cache: "no-cache" });
    if (!response.ok) throw new Error(tr("网页数据暂不可用"));
    const content = await response.text();
    const digest = await crypto.subtle.digest("SHA-256", new TextEncoder().encode(content));
    const hash = [...new Uint8Array(digest)].map(byte => byte.toString(16).padStart(2, "0")).join("");
    if (hash !== expected.sha256) throw new Error(tr("科技树版本校验失败，请刷新页面后重试"));
    const raw = JSON.parse(content);
    if (!Array.isArray(raw)) throw new Error(tr("科技树数据格式错误"));
    return { success: true, country, type, data: raw };
  }

  throw new Error(tr("网页版使用随发布更新的科技树数据"));
}
...
function runExactPlan() {
  ...
  window.setTimeout(() => {
    try {
      const hiddenIds = state.units
        .filter(unit => window.RosterAudit?.isHidden(state.country, state.type, unit.data_unit_id))
        .map(unit => unit.data_unit_id);
...
function renderSummary() {
  ...
  document.getElementById("totalRpLabel").textContent = state.missing.some(unit => unit.rp == null) ? tr("已知 RP") : tr("还需 RP");
  document.getElementById("totalSpLabel").textContent = state.missing.some(unit => unit.sp == null) ? tr("已知 SL") : tr("购买 SL");
...
function renderTree() {
  ...
  window.RosterAudit?.render(state.country, state.type);
  if (!state.tree.length) {
    els.treeContainer.innerHTML = `<div class="loading">${tr("Wiki 暂无该国家的此类科技树")}</div>`;
    return;
  }
...
async function loadMeta() {
  ...
  if (!response.ok) throw new Error(tr("数据核验清单暂不可用"));
